...
export default function SignUpPage() {
  ...
  const inputStyle = { border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', background: 'var(--surface-card)', color: 'var(--text-primary)', padding: '0.82rem 0.9rem', font: 'var(--type-body-md)' };
      ...
      <div style={{ width: '100%', maxWidth: 560 }}>
        <form onSubmit={handleSubmit} style={{ background: 'var(--surface-card)', border: '1px solid var(--border-hairline)', borderRadius: 'var(--radius-card)', boxShadow: 'var(--shadow-md)', padding: 'var(--space-5)', display: 'grid', gap: 'var(--space-4)' }}>
          <div style={{ display: 'grid', gap: 'var(--space-2)' }}>
            <span style={{ display: 'inline-flex', width: 'fit-content', alignItems: 'center', gap: '0.4rem', borderRadius: 'var(--radius-pill)', background: 'var(--brand-subtle)', color: 'var(--brand)', padding: '0.45rem 0.7rem', font: 'var(--type-label)' }}><ShieldCheck size={14} /> Créer un compte</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', borderRadius: 'var(--radius-card-inner)', background: 'var(--brand-subtle)', border: '1px solid var(--brand-border)', color: 'var(--brand)', padding: '0.8rem 0.9rem', font: 'var(--type-body-sm)' }}><CheckCircle2 size={16} /> Votre identité est sécurisée par Firebase.</div>
          ...
          <div style={{ textAlign: 'center', font: 'var(--type-body-sm)', color: 'var(--text-muted)' }}>Vous avez déjà un compte ? <button type="button" onClick={() => router.push('/sign-in')} style={{ border: 0, background: 'transparent', color: 'var(--brand)', padding: 0, font: 'inherit', cursor: 'pointer' }}>Se connecter</button></div>
        </form>
      </div>
    </main>
  );
