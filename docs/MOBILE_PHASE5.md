...
| Messaging | Implemented, not run on a device | Inbox pagination, unread badge polling, thread history opening on the newest page with older pages above, a 5 s foreground poll for new replies (`after=`) and read receipts, mark-read, guarded send button, draft-preserving send errors, and deep links for conversations. |
