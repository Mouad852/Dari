...
| Deep links | `dari://listings/{id}` (and `listing/{id}`), `dari://messages/{id}` (and `conversation/{id}`), `dari://profile-recovery` → sign-in, rewritten in `app/+native-intent.tsx` before expo-router matches the URL. | `src/lib/deepLinks.ts`, `app/+native-intent.tsx` |
