...
- **Ownership and role checks verified across mutating API routes (2026-09-05)**: profile-backed listing, favorite, messaging, report, and user writes now carry explicit `USER`/`ADMIN` method-security guards, while service-level owner, participant, and user scoping remains authoritative. Focused HTTP regressions cover cross-user listing/photo mutation, favorite removal, non-participant messaging, and profileless writes. Soft-delete and backup work remain out of scope for this slice.
