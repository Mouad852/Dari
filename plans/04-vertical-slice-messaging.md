...
- [~] Conversations about `SUSPENDED` or `EXPIRED` listings stay accessible — the conversation itself is unaffected by listing status (no FK cascade, no status check anywhere in `ConversationService`), but nothing marks the listing "unavailable" in the payload the way §4 asks; the frontend's listing context card just fetches `GET /listings/{id}` directly and silently hides itself if that 404s (e.g. soft-deleted), it doesn't render an explicit "unavailable" state for a merely-suspended one
...
- [~] Listing context card inside the thread — shown when the listing still loads publicly; **no unavailable state**, see the backend note above. It's a working card, not the specified degraded state.
