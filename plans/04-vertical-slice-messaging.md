...
- [x] **Unread badges — built 2026-09-10.** Added the backend aggregate this item's own note called for: `MessageRepository.countUnreadForUser` (same query shape as the existing `findUnreadForUser` that backs `markRead`) and a new `unreadCount` field on `ConversationResponse`. The inbox now shows a real numeric badge per conversation and bolds the row for unread threads; both clear correctly on open, via the same `PATCH .../read` call that already existed. See `TODO.md`'s "Messaging and account" section for the full verification writeup.
