...
- [x] Rooms read contract (2026-09-08) — `rooms: ListingRoomResponse[]` (id, roomType, isRentable, isShared, description) now appears on `GET /listings/{id}` and on the owner-facing `ListingResponse` (`/listings/mine`, `/listings/mine/{id}`, create/update, admin approve/reject). Unlike `houseRules`, an empty room list serializes as `[]`, never `null` — there is no "row missing entirely" ambiguity for a one-to-many collection the way there is for a 1:1 row. The four lifecycle actions (submit/renew/mark-room-found/reopen) pass `null` for rooms, following the existing `coverPhotoUrl`/`houseRules` precedent for a lifecycle confirmation that isn't re-rendering a card.
- [x] Rooms write contract (2026-09-08) — `POST /listings` and `PATCH /listings/{id}` accept a `rooms` array (`ListingRoomRequest`: `roomType` required, `isRentable`, `isShared`, optional `description`). Full replace: omitting the field leaves existing rooms untouched, an explicit `[]` clears every room, and a new list deletes all existing rows and reinserts — the same rule `amenityCodes` uses, for the same reason (the wizard's "Pièces" step submits the whole room list every time, so there is no stable per-room identity from the client to diff against). A room missing `roomType` is refused with a 400. The wizard step that fills this landed the same day — see the frontend list below.
...
- Rooms round-trip through the whole system now (2026-09-08): API (read, create, patch — `ListingApiTest.roomsRoundTripOnCreateAndUpdate`) and the wizard's "Pièces" step, verified against a real published listing.
- The production wizard matches the prototype at 375px and 1440px — the live wizard has 6 steps, not the prototype's 8; never compared side by side

## Risks and open decisions

- **This is the largest phase in the plan.** The backend write model is substantially complete; house rules, rooms, map selection, and city-membership validation are all now done end to end. What remains is full 8-step prototype parity (the condensed 6-step wizard is functionally complete but has never been checked against the prototype's responsive transformations or exact field set) and per-step/responsive validation.
