...
- [ ] Repeatable room list, with the salon-as-bedroom case working — no UI yet; the wizard still has only singular `numBedrooms`/`numBathrooms` fields. Backend structure, read contract, and write contract all landed 2026-09-08 (`V21`, `ListingRoom`, `ListingRoomRepository`, `rooms` on both the read and write side of `/listings`); only the wizard's "Pièces" step remains.
...
- Rooms round-trip through the API in full (2026-09-08): read, create, and patch all work, verified by `ListingApiTest.roomsRoundTripOnCreateAndUpdate`. No UI writes to it yet — the wizard's "Pièces" step is the only remaining piece of this feature.
