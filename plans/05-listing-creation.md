...
- [ ] Repeatable room list, with the salon-as-bedroom case working — no UI yet; the wizard still has only singular `numBedrooms`/`numBathrooms` fields. Backend structure and read contract both landed 2026-09-08 (`V21`, `ListingRoom`, `ListingRoomRepository`, `rooms` on `GET /listings/{id}` and the owner-facing response); write contract and the "Pièces" step remain.
...
- Rooms round-trip — the read side does (`rooms` on `GET /listings/{id}` and the owner-facing response, 2026-09-08), but nothing writes a room yet: no service method, no endpoint, no UI.
