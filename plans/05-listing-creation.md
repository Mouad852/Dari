...
- [x] Photo upload: `POST/PATCH/DELETE /listings/{id}/photos`, with `sort_order` and `is_cover`; the public detail response now reads the ordered active photos back for display.
...
- [ ] Photo upload with drag-reorder and cover selection; first photo is the cover — not verified against the current wizard's "Photos" step; check before assuming.
