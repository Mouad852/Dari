...
- [x] Cap map-pin response size (2026-09-05): a search load test at 50k seeded listings (~12.5k/city) found the map endpoint had no result limit at all — an unfiltered city request serialized every published listing in it, p95 1.79s and multi-gigabyte total payload under concurrent load. `mapPinsByLocationAndRadius` now caps at 1,000 rows ordered by recency (`ListingSearchService.MAP_PIN_LIMIT`); post-fix p95 is 201ms. Neighborhood clustering (deferred above) remains the real long-term answer for a city whose true inventory exceeds the cap — this is a safety valve, not a substitute for that decision. Full methodology and before/after numbers in `docs/PRODUCTION_OPERATIONS.md`.
