...
- rate limits are configurable through `DARI_RATE_LIMIT_*` environment variables; expired local windows are swept on every JVM. Redis-backed limiting and an explicitly configured trusted-proxy boundary are mandatory before horizontal API scaling; forwarded headers are not trusted by default
