...
- rate limits are configurable through `DARI_RATE_LIMIT_*` environment variables and tracked keys are bounded by `DARI_RATE_LIMIT_MAX_TRACKED_KEYS`; expired local windows are swept on every JVM and overflow shares a restrictive bucket. Listing detail and public reference reads use the separate generous `SSR_READ` shared ceiling because Next.js Server Components share the web runtime address; authenticated callers also retain a Firebase-user dimension, while anonymous SSR per-visitor fairness is a follow-up. In production Tomcat accepts forwarded headers only from `DARI_TRUSTED_PROXY_IPS` (the ALB target-facing range), relies on ALB's default append mode, and preserves the original HTTPS scheme. Redis-backed limiting remains mandatory before horizontal API scaling
