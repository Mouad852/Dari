...
- listing detail pages are server-rendered with per-listing metadata, canonical URLs, Open Graph tags and schema.org `Accommodation` markup; an unavailable listing is served `noindex, nofollow` and leaves the sitemap, while the page itself still renders so its owner and a moderator can preview it
- `sitemap.xml` carries listing URLs from the published view, and both the sitemap batches and `robots.txt` export `revalidate = 3600`, so a listing published after a deploy enters the index within the hour instead of waiting for the next build
- public profile pages carry their own title and canonical but are `noindex, follow`: a member's display name, city and bio exist to be judged inside the product, not to be indexed. A deleted, banned or unknown profile returns a real 404
...
- rate limits are configurable through `DARI_RATE_LIMIT_*` environment variables and tracked keys are bounded by `DARI_RATE_LIMIT_MAX_TRACKED_KEYS`; expired local windows are swept on every JVM and overflow shares a restrictive bucket. Next.js server renders share the web runtime's address, so reads carrying the server-only `DARI_SSR_SHARED_SECRET` (header `X-Dari-Ssr-Key`, sent by `apiFetch` on the server only, compared in constant time) draw on one generous shared `SSR_READ` ceiling plus a Firebase-user window when authenticated; every other caller — browsers, the mobile app, a missing or wrong key — gets the per-address (and per-user) `SEARCH` limit, even on `SSR_READ` endpoints such as listing detail, and mutation limits ignore the header. Per-visitor fairness for anonymous SSR traffic is a deferred follow-up (anyone who can drive many renders can still spend the shared budget), and the per-address `SEARCH` limit can be tight behind carrier NAT (`DARI_RATE_LIMIT_SEARCH_MAX` is tunable). In production Tomcat accepts forwarded headers only from `DARI_TRUSTED_PROXY_IPS` (the ALB target-facing range), relies on ALB's default append mode, and preserves the original HTTPS scheme. Redis-backed limiting remains mandatory before horizontal API scaling
- media stays local by default for development and can use the S3-compatible adapter with MinIO or production object storage through `DARI_MEDIA_*`; deleted media is queued in the durable cleanup outbox and retried
