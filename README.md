...
- account deletion also clears personal data from the row, not just its `deleted_at` flag: email, phone, first name, city, bio, and the stored avatar file (deleted from disk, not just unlinked) are cleared; `displayName` becomes a fixed "Utilisateur supprimé" placeholder since the column can't be null. This applies only to self-deletion, not to a banned or suspended account, whose data remains legitimate moderator audit material
