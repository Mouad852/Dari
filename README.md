...
- **search result cards cannot show photos** — `PublicListingResponse` carries no photo field, so the feed renders a placeholder for every listing. Needs a cover-photo URL on the search DTO before any frontend fix
- **the search filter rail is not responsive** — a hard-coded 300px column whose contents measure 383px, so the panel is clipped on desktop and the page scrolls horizontally on mobile. This is what phase 07's filter UI work has to fix
...
- the admin report queue names its target, shows the reporters' prior-dismissal history, and supports dismiss, suspend (listing or account) and ban (accounts only) as real actions that close the reports with the correct outcome. **Warn is still refused on purpose** — its only effect is notifying the owner, and notification delivery does not exist yet (phase 10); recording a warning nobody received would be worse than refusing it
