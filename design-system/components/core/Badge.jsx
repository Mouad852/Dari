...
export function Badge({children,tone='brand',icon,size='md',style,...rest}){
  const t=TONES[tone]||TONES.brand;const sm=size==='sm';
  return <span {...rest} style={{display:'inline-flex',alignItems:'center',gap:4,
    padding:sm?'2px 8px':'4px 10px',background:t.bg,color:t.fg,border:`1px solid ${t.bd}`,
    borderRadius:'var(--radius-pill)',font:`var(--weight-semibold) ${sm?'var(--text-micro)':'var(--text-caption)'}/1.3 var(--font-ui)`,...style}}>
    {icon&&<Icon name={icon} size={sm?11:13}/>}{children}</span>;
}
