...
| **Logo** | **provided** (6 JPEGs, this turn) — cleaned to transparent PNGs in `assets/` |
| **Photography** | **provided** (6 interiors, this turn) — cropped into `assets/photos/`; see brand-fit caveat in `assets/README.md` |
| Icon set, font binaries | none provided |
...
- ~~The name "Dari"~~ — **confirmed by the client's logo files.**
...
- ~~No logo exists.~~ **Resolved** — the client supplied the mark; see Assets below. Nothing was drawn from memory at any point.
...
- ~~No photography.~~ **Partly resolved** — six client interiors are in. Three of the six are cool-toned stock that contradicts the warm-daylight rule, and none is recognisably Moroccan; treat them as structural, not final.
...
**The logo is the client's own**, supplied as JPEGs and cleaned into transparent PNGs — lockup and symbol, each in terracotta, cream and charcoal. See `assets/README.md` for the file table and usage rules.

The mark is **two joined rooflines sharing a gable** — two homes in one, the literal picture of colocation. It reads as an outline drawn in a single terracotta stroke with round caps, which is why Lucide's round-cap stroke icons sit beside it so naturally: **the icon set and the logo share a drawing logic.** Keep that alignment — never pair the mark with filled or sharp-cornered glyphs.

Wordmark: lowercase geometric sans, tight tracking, no ascenders above the roofline. Clear space = one roof height. Lockup down to 120px wide, symbol alone below that.

Photography now lives in `assets/photos/` (six client interiors, cropped per surface). Still missing: **vector logo originals** — the supplied rasters are 720px and will soften at large or print sizes — and **warm, Moroccan, people-present photography** to replace the cool stock interiors.
...
| `templates/` | Starting folders consuming projects copy — `landing-page/` (marketing homepage) and `room-app/` (mobile feed screen) |
