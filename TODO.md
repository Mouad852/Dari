...
- House rules step 1 of 4 (2026-09-05): `HouseRules` entity + `HouseRulesRepository` added over the `house_rules` table V12 created back in phase 05 and nothing ever used. No read or write contract yet — that is steps 2 and 3. Mapped with `@MapsId` over a scalar `listingId`, **not** `@Id` directly on the association: the first attempt copied `ListingAmenity`/`Favorite`'s `@Id @ManyToOne` shape, but those carry a *composite* key and therefore an `@IdClass`, and Spring Data rejects a single-association `@Id` without one ("This class does not define an IdClass") at repository-bean creation. It compiled fine and failed every integration test at context startup instead. Verified via `cd apps/api && ./mvnw test` (119/119; `ddl-auto: validate` is what proves the column mapping matches the real V12 table, since a wrong column name fails the whole suite at boot).
